package att.moe.fluttervediocompress
...
class FlutterVedioCompressPlugin: MethodCallHandler {
  private val channelName = "flutter_vedio_compress"
  ...
  companion object {
    ...
    fun registerWith(registrar: Registrar) {
      val channel = MethodChannel(registrar.messenger(), "flutter_vedio_compress")
      channel.setMethodCallHandler(FlutterVedioCompressPlugin())
      reg = registrar
    }
  }
  ...
  fun compressVedio(path: String, deleteOrigin: Boolean, result: Result) {
    if (TextUtils.isEmpty(path)) {
      return
    }

    val lastIndex = path.lastIndexOf("/")
    val newPath = path.substring(0, lastIndex) + "/flutter_vedio_compress" + path.substring(lastIndex)
